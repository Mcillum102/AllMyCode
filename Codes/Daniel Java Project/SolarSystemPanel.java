...
public class SolarSystemPanel extends JPanel implements Runnable {
    ...
    public SolarSystemPanel() {
            ...
            public void mouseClicked(MouseEvent e) {
                for (Planet planet : planets) {
                    ...
                    if (Math.hypot(e.getX() - planetX, e.getY() - planetY) < 20) {
                        ImageIcon planetIcon = new ImageIcon(planet.getImagePath());
                        JLabel messageLabel = new JLabel(
                                "<html>Planet: " + planet.getName() + "<br>" +
                                        "Orbit Speed: " + planet.getSpeed() + "<br>" +
                                        "Orbit Distance: " + planet.getDistance() + "</html>");
                        messageLabel.setFont(new Font("Dialog", Font.PLAIN, 20));

                        JOptionPane.showMessageDialog(SolarSystemPanel.this,
                                messageLabel,
                                planet.getName() + " Info", JOptionPane.INFORMATION_MESSAGE, planetIcon);
                        break;
                    }
                }
            }
        ...
        sliderPanel.setPreferredSize(new Dimension(200, 70));
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (Planet planet : planets) {
            int orbitRadius = (int) (planet.getDistance() * zoomFactor);
            g2d.drawOval(centerX - orbitRadius, centerY - orbitRadius, 2 * orbitRadius, 2 * orbitRadius);
        }
        ...
        for (Planet planet : planets) {
            ...
            g2d.drawImage(planet.getImage(), planetX - planetSize / 2, planetY - planetSize / 2, planetSize, planetSize,
                    null);
